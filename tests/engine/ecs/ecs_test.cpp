...
TEST_P(EntityManagerTest, EntityRemovedListener) {
    ...
    manager.getEntitySet<Position, Velocity, Mass>().addEntityAddedListener(
            [&counterMass]([[maybe_unused]] auto entity) {
                ++counterMass;
            });
    manager.getEntitySet<Position>().addEntityRemovedListener([&counterPosition]([[maybe_unused]] auto entity) {
        --counterPosition;
    });
    manager.getEntitySet<Position, Velocity>().addEntityRemovedListener(
            [&counterVelocity]([[maybe_unused]] auto entity) {
                --counterVelocity;
            });
    manager.getEntitySet<Position, Velocity, Mass>().addEntityRemovedListener(
            [&counterMass]([[maybe_unused]] auto entity) {
                --counterMass;
            });
    for (auto i = std::size_t(0); i < nbEntities; ++i) {
